    ...
    tabPanel("Tratamento de dados",
             sidebarLayout(
               sidebarPanel(column(12,
                                   #radioButtons("in_tp_analysis", "Escolha o tipo de anÃ¡lise:",
                                   isolate(selectInput("in_tp_analysis", "Escolha o tipo de análise:",
                                                       list(`COLABORAÇÃO` = c("Colaboração Autores", "Colaboração Países", "Colaboração Instituições"),
                                                            ...
                                                            `CO-CITAÇÃO` = c("Co-citação Documentos", "Co-citação Autores", "Co-citação Fontes"),
                                   ...
                                   isolate(selectInput("in_tp_layout", "Escolha o layout:",
                                                       c("Fruchterman-Reingold" ="layout.fruchterman.reingold",
...
server <- function(input, output) {
  ...
  observeEvent(input$in_cria_rede,{
    output$out_table_graph = renderDataTable({
      dd <-as.data.frame(diag(my_NetMatrix))
      #colnames(dd)<-c("Frequencia")
      #dd[order1,]
      #dd[order(-dd$degree),]
  ...
  observeEvent(input$in_cria_rede, {
    output$out_plot_net = renderPlot({
      ...
      if (isolate (input$in_tp_dim)=="in"){
        V(my_graph)$size<-V(my_graph)$indegree * 0.6  
      }
      else if (isolate (input$in_tp_dim)=="out"){
        V(my_graph)$size<-V(my_graph)$outdegree * 0.6 
            }
      else {
              V(my_graph)$size<-V(my_graph)$degree * 0.6 
      }
      
      if (isolate (input$in_tp_gigante)==TRUE){
        cl = clusters(my_graph)
        comp_gigante = which(cl$membership == which.max(cl$csize))
        V(my_graph)$color  = "#4682B4"
      ...
      if (isolate (input$in_tp_cluster)==TRUE){
        ...
        #my_coords<-norm_coords(coords, xmin = -1, xmax = 1, ymin = -1, ymax = 1,
        #                       zmin = -1, zmax = 1)
        plot(my_graph,
