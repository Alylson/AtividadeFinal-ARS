               ...
               sidebarPanel(column(12,
                                   radioButtons("in_tp_metrica", "Escolha a métrica:",
                                                c("Grau médio" = "netdegree",
                                                  ...
                                                  "Comprimento médio de caminho" = "path")), # troquei pq estava strength...
                                   #"Componentes conectados" = "collaborationUniversities",
...
server <- function(input, output) {
 ...
 observeEvent(input$show6, {
  ...
  #Estat�sticas de For�a dos vertices#
  
  output$out_plot_degree <- renderPlot({
    if (input$in_tp_metrica=="strength"){
      #hist(V(my_graph)$degree,col="lightblue",xlim=c(0, max(V(my_graph)$degree)),xlab="For�a dos vértices", ylab="Frequência", main="", axes="TRUE")
      hist(my_graph_metrics$strength,col="lightblue",xlim=c(0, max(my_graph_metrics$strength)),xlab="For�a dos vértices", ylab="Frequência", main="", axes="TRUE")
      legend("topright", c(paste("M�?n.=", round(my_graph.strength.summary[1],2)),
             ...
             pch = 1, title = "For�a dos Vertices")
    }
  })
  
  output$out_plot_indegree <- renderPlot({
    if (input$in_tp_metrica=="strength"){
      hist(my_graph_metrics$instrength,col="lightblue", xlab="For�a de entrada", ylab="Frequência", main="", axes="TRUE")
  ...
  output$out_plot_outdegree <- renderPlot({
    if (input$in_tp_metrica=="strength"){
      hist(my_graph_metrics$outstrength,col="lightblue", xlab="For�a de sa�?da", ylab="Frequência", main="", axes="TRUE")
  ...
  output$out_plot_boxdegree <- renderPlot({
    if (input$in_tp_metrica=="strength"){
      boxplot(my_graph_metrics["instrength"], my_graph_metrics["outstrength"], my_graph_metrics["strength"], notch = FALSE, ylab = 'For�a', 
 ...
 # Page Rank da rede #
  ...
  # Componentes Conectados #
   ...
   # if (input$in_tp_metrica=="collaborationUniversitie"){  
   #    output$out_tp_metrica <- renderText({paste("Componentes Conectados da Rede : ",)
   #    })
   #  }
  ...
  if (input$in_tp_metrica=="clustering"){  
    output$out_tp_metrica <- renderText({paste("Coeficiente de Clustering M�dio: ",my_graph_metrics$clustering)
    })
  }
  })
